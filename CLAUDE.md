...
**Essential Sections**:
- [🌐 Open Source Status](#-open-source-status-feb-2025---current) - Public repo, security audit, contributing guidelines
...
- [Architecture Decision Records](#architecture-decision-records) - ADR-001 through ADR-035

**Key ADRs** (35 total):
- [ADR-035: Open Source Security Remediation](#adr-035-open-source-security-remediation---all-15-public-repos-feb-21-2026) - Security audit across all public repos
...
## 🌐 Open Source Status (Feb 2025 - Current)

### Public Repository
CoCo is open-sourced at **[github.com/keef75/CoCo](https://github.com/keef75/CoCo)** under the MIT License.

**Git Remotes**:
- `github` → `https://github.com/keef75/CoCo.git` (public open-source repo)
- `origin` → `https://github.com/cocoa-labs/CoCo-keef.git` (private dev repo)

**Push to public**: `git push github main`

### Security Audit (Feb 2025) - All 15 Public Repos Clean
A comprehensive security audit was performed across all 15 public repos on the keef75 GitHub profile. All repos are now clean:
- **13 repos** were already clean (proper `.env.example`, `os.getenv()`, `.gitignore`)
- **2 repos** were remediated:
  - `MYGPT3-to-11-TTS` (commit `e35d75a`): Replaced hardcoded API key placeholders with `os.getenv()` + dotenv, added `.env.example` and `.gitignore`
  - `cocoa-template` (commit `94faa16`): Replaced hardcoded Django `SECRET_KEY` with `os.environ.get()`, added `.env.example`, added `.env` to `.gitignore`

### Security Practices
- **No secrets in code**: All API keys loaded via `.env` / `os.getenv()` / `os.environ.get()`
- **`.gitignore`** excludes: `.env`, `token.json`, `credentials.json`, `*.db`, `coco_workspace/`, `venv_cocoa/`
- **`.env.example`** provides placeholder template for required keys
- **Never commit**: Real API keys, OAuth tokens, database files, or user workspace data

### Resolved (Feb 2025)
- **README.md** clone URLs updated from `Nova-ACE` → `CoCo` (lines 182, 633, badge link)
- **LICENSE** file created with MIT License text (referenced by README line 619)
...
### ADR-035: Open Source Security Remediation - All 15 Public Repos (Feb 21, 2026)

**Context**: With CoCo open-sourced at github.com/keef75/CoCo, a comprehensive security audit was performed across all 15 public repos on the keef75 GitHub profile to ensure no exposed secrets, hardcoded credentials, or poor security practices.

**Audit Results**: 13 of 15 repos were already clean. Two repos needed remediation:

**Fix 1: MYGPT3-to-11-TTS** (commit `e35d75a`):
- **Problem**: `GPT3 text to voice-Copy1.py` had hardcoded API key placeholders (`'Your OpenAI API here'`, `'Your ElevenLabs API here'`) with no env var pattern
- **Fix**: Replaced with `os.getenv('OPENAI_API_KEY')` and `os.getenv('ELEVENLABS_API_KEY')` using python-dotenv
- **Added**: `.env.example` with placeholder values, `.gitignore` excluding `.env`/`*.mp3`/`__pycache__/`
- **Bonus**: Cleaned up imports, added key validation with error messages, removed dead code

**Fix 2: cocoa-template** (commit `94faa16`):
- **Problem**: `cocoa_template/settings.py` had hardcoded Django `SECRET_KEY`: `django-insecure-rgvi*=j#uxty...`
- **Fix**: Replaced with `os.environ.get('DJANGO_SECRET_KEY', 'django-insecure-change-me-in-production')`
- **Added**: `.env.example` with `DJANGO_SECRET_KEY=your-secret-key-here`, `.env` added to `.gitignore`

**Clean Repos** (no changes needed):
CoCo, agent-civics, agent-civ-poc, 10-nodes-math-brain, SEFAS, SEFAS-CIVILIZATION, SEFAS-GPT5, Realtime-avatar, hello-world, ARK-AGI-2, Agent-Image-analysis, takeoff-cursor-course, Time-is-Change

**Security Pattern Reference**:
- All repos now follow: env vars for secrets, `.env.example` for templates, `.gitignore` for exclusion
- CoCo itself was already clean: proper `.env.example`, comprehensive `.gitignore`, `os.getenv()` throughout

**Result**: All 15 public repos on keef75 GitHub profile are now clean with no exposed secrets or hardcoded credentials. ✅
